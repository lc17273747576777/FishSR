...
class RFDN(nn.Module):
    def __init__(self, in_nc=3, nf=50, num_modules=4, out_nc=3, upscale=4):
        ...
        # upsample_block = torch.nn.functional.interpolate()
        self.upsampler = upsample_block(nf, out_nc, upscale_factor=upscale)
        self.scale_idx = 0

        ###
        self.outlayer = nn.Sequential(nn.ReflectionPad2d(1), nn.Conv2d(nf, out_nc, 3))
        ###
    def forward(self, input):
        ...
        output = torch.nn.functional.interpolate(out_lr, scale_factor=self.scale, mode='bicubic')
